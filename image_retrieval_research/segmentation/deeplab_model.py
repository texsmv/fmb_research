...
class DeepLabModel(object):
  ...
  def __init__(self, tarball_path):
    ...
    self.sess = Session(graph=self.graph, config=session_config)
  ...
  def getMaskFromClass(self, img, class_number):
    img, seg_img = self.run(img)
    ...
    return img, seg_img.astype(np.uint8)
